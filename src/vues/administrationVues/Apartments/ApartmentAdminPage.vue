...
const fetchApartments = async () => {
  error.value = null;
  try {
    const response = await axiosInstance.get('/appartements');
...
const filteredApartments = computed(() => {
  ...
  return apartment.value.filter(apartment => apartment.type === selectedRealEstateType.value);
});
...
const viewApartmentDetails = (userId) => {
...
const confirmDelete = async () => {
  try {
    const apartment = apartment.value.find(a => a.appartement_id === apartmentIdToDelete.value);
    await axiosInstance.delete(`/appartements/${apartmentIdToDelete.value}/`);
    deletedApartmentDetails.value = { id: apartment.appartement_id, owner: apartment.owner, location: apartment.location, };
  ...
  <div class="ui container" style="min-height: 100vh; display: flex; flex-direction: column;">
    ...
    <div class="ui basic segment flex-grow" style="flex: 1 0 auto; overflow: auto;">
      <div class="header-container">
        <h1 class="ui header">{{$t('apartment_administration')}}</h1>
        <div class="ui right aligned">
          <div class="custom-dropdown">
            <select v-model="selectedRealEstateType">
          ...
          <button class="ui primary button" @click="navigateToAddApartment">
            <i class="plus icon"></i>
          </button>
        </div>
      </div>
      ...
      <div v-else>
        <table class="ui celled table">
          ...
          <tr v-for="user in paginatedUsers" :key="user.users_id">
            ...
            <td>{{ user.email }}</td>
            <td>
              <button class="ui icon button" @click="viewApartmentDetails(user.users_id)">
                <i class="eye icon"></i>
              </button>
    ...
    <div v-if="showSuccessModal" class="modal-overlay">
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <p><strong>Rôle:</strong> {{ deletedApartmentDetails.role }}</p>
